    ...
    with _lock:
        if os.path.exists(USAGE_FILE):
            try:
                ...
                pass
        return {"keys": [], "limit_per_key": 50, "date": str(datetime.date.today()), "usage": {}}
...
def get_keys_status():
    ...
    return {"keys": status, "limit_per_key": data.get("limit_per_key", 50)}

def set_keys_settings(keys, limit_per_key):
    data = load_data()
    _check_reset(data)
    data["keys"] = keys
